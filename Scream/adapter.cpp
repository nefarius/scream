...

{
    ...
    parametersPath.Buffer = (PWCH)ExAllocatePoolWithTag(NonPagedPoolNx, parametersPath.MaximumLength, SCREAM_POOLTAG);
    ...
    if ((unicastIPv4.Length > 0) && RtlUnicodeStringToAnsiSize(&unicastIPv4)) {
        g_UnicastIPv4 = (PCHAR)(ExAllocatePoolWithTag(NonPagedPoolNx, RtlUnicodeStringToAnsiSize(&unicastIPv4) + 1, SCREAM_POOLTAG));
        if (g_UnicastIPv4) {
    ...
    if (g_UnicastIPv4 == NULL) {
        g_UnicastIPv4 = (PCHAR)(ExAllocatePoolWithTag(NonPagedPoolNx, 16, SCREAM_POOLTAG));
        if (g_UnicastIPv4) {
...
// same as above for source IP
	if ((unicastSrcIPv4.Length > 0) && RtlUnicodeStringToAnsiSize(&unicastSrcIPv4)) {
		g_UnicastSrcIPv4 = (PCHAR)(ExAllocatePoolWithTag(NonPagedPoolNx, RtlUnicodeStringToAnsiSize(&unicastSrcIPv4) + 1, SCREAM_POOLTAG));
		if (g_UnicastSrcIPv4) {
			ANSI_STRING asString;
			asString.Length = 0;
	...
	if (g_UnicastSrcIPv4 == NULL) {
		g_UnicastSrcIPv4 = (PCHAR)(ExAllocatePoolWithTag(NonPagedPoolNx, 16, SCREAM_POOLTAG));
		if (g_UnicastSrcIPv4) {
